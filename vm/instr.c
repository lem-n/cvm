...
void instr_iconst(vm_container* vm) {
    ...
    vm->stack[++vm->stack_ptr] = value;        // increase stack pointer so we're on
                                               // the top, and then set the value
}
...
void instr_getc(vm_container* vm) { vm->stack[vm->stack_ptr++] = fgetc(stdin); }
