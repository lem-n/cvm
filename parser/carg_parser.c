...
static char parse_long_option(struct Arg_parser* const ap, const char* const opt,
                              const char* const arg, const struct ap_Option options[],
                              int* const argindp) {
    unsigned len;
    int index = -1, i;
...
static char parse_short_option(struct Arg_parser* const ap, const char* const opt,
                               const char* const arg, const struct ap_Option options[],
                               int* const argindp) {
...
char ap_init(struct Arg_parser* const ap, const int argc, const char* const argv[],
    ...
    while (argind < argc) {
        ...
        if (ch1 == '-' && ch2) /* we found an option */
        {
            ...
            if (!in_order) {
                void* tmp =
                    ap_resize_buffer(non_options, (non_options_size + 1) * sizeof *non_options);
                if (!tmp)
                    return 0;
...
const char* ap_error(const struct Arg_parser* const ap) { return ap->error; }

int ap_arguments(const struct Arg_parser* const ap) { return ap->data_size; }
